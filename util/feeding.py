...
def samples_to_spectrogram(samples, sample_rate, train_phase=False):
    ...
    #spectrogram = contrib_audio.mfcc(spectrogram, sample_rate, dct_coefficient_count=Config.n_input)
    spectrogram = tf.reshape(spectrogram, [-1, Config.n_input])

    return spectrogram, tf.shape(input=spectrogram)[0]


def audiofile_to_features(wav_filename, train_phase=False):
    samples = tf.io.read_file(wav_filename)
    decoded = contrib_audio.decode_wav(samples, desired_channels=1)
    features, features_len = samples_to_spectrogram(decoded.audio, decoded.sample_rate, train_phase=train_phase)
...
def split_audio_file(audio_path,
    ...
    def to_spectrogram(time_start, time_end, samples):
        features, features_len = samples_to_spectrogram(samples, sample_rate)
    ...
    def create_batch_set(bs, criteria):
        return (tf.data.Dataset
                ...
                .map(to_spectrogram, num_parallel_calls=tf.data.experimental.AUTOTUNE)
                .filter(criteria)
